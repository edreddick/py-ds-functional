...
## uncomment these after have created a StepMajority and added to pre-processing
...
## re-write this so that instances are called inside the pipe
## because there's no need to pollute the namespace
...
## the folds should be set up with respect to the proportion of the response (still needed)
inner_cv = KFold(n_splits=4, shuffle=True, random_state=1)
outer_cv = KFold(n_splits=4, shuffle=True, random_state=1)

## specify potential hyper-parameters
## can also add choices from pre-processing
## ex) 'ct__num__si_n__strategy' : ['mean', 'median']
...
clf = GridSearchCV(pipe, param_grid=param_grid, cv=inner_cv, n_jobs= 1)
...
print(clf.best_score_) # do not use this to quote model performance, overly optimistic 
print(clf.best_params_)

## Step 5: eval final model on testing data
## according to the documentation this is the percentage of deviance explained
## missing weight
#clf.score(Xtest, ytest, sample_weight=swtest)
